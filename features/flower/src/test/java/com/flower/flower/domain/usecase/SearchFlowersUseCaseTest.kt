...
class SearchFlowersUseCaseTest {

    @get:Rule
    val testInstantTaskExecutorRule: TestRule = InstantTaskExecutorRule()
    private val testCoroutineDispatcher = TestCoroutineDispatcher()

    private lateinit var useCase: SearchFlowersUserCase
    ...
    fun setUp() {
        ...
        useCase = SearchFlowersUserCase(fakeRepo)
    }

    @Test
    fun `Given useCase, when searchFlowers, then get loading - success`() =
        runBlockingTest {
            val expected = mapper.map(fakeData.getFlowersListResponse())

            val resultLiveData = useCase.execute("test").asLiveData().test()
    ...
    @Test
    fun `Given useCase, when searchFlowers, then loading - error`() =
        runBlockingTest {
            val errorCode = NetworkCodes.CONNECTION_ERROR
            val errorScript = "Failed to connect to server please check you network!"

            coEvery { fakeRepo.searchFlowers("test") } returns fakeData.getFailureEmit()
